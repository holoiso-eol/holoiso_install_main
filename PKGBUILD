...
package() {
    holoiso_basever="SteamOS_Holo-${BTIME}_amdgpu-x86_64"
    holoiso_codename="${HOLOCODENAME}"
    ...
    cp "${srcdir}/pacman.conf" "${srcdir}/etc/pacman.conf"
    sed -i "s/snapshotver/snapshot${BTIME}/g" osinfo_tmp
